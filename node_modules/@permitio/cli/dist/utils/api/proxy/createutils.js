 ...
 */
export function validateProxyConfig(options) {
    const errors = [];
    ...
    if (!allowedAuth.includes(options.auth_mechanism)) {
        errors.push('Validation Error: auth_mechanism must be one of Bearer, Basic, or Headers');
    }
    // Validate secret based on auth_mechanism
    switch (options.auth_mechanism) {
        case 'Bearer':
            if (typeof options.secret !== 'string' || !options.secret.trim()) {
                errors.push('Validation Error: Bearer secret must be a non-empty string');
            }
            break;
        case 'Basic':
            if (typeof options.secret !== 'string' ||
                !options.secret.trim() ||
                !/.+:.+/.test(options.secret)) {
                errors.push('Validation Error: Basic secret must be a string of the form "username:password"');
            }
            break;
        case 'Headers':
            if (typeof options.secret !== 'object' ||
                options.secret === null ||
                Array.isArray(options.secret)) {
                errors.push('Validation Error: Headers secret must be an object of headers');
            }
            else {
                Object.entries(options.secret).forEach(([hdr, val]) => {
                    if (typeof val !== 'string') {
                        errors.push(`Validation Error: secret header '${hdr}' must be a string`);
                    }
                });
            }
            break;
        ...
        if (!Array.isArray(options.mapping_rules)) {
            ...
            options.mapping_rules.forEach((rule, idx) => {
                ...
                if (!rule.resource?.trim() || !keyRegex.test(rule.resource)) {
                    errors.push(`Validation Error: ${base}.resource is invalid ('${rule.resource}')`);
                }
                if (rule.headers !== undefined) {
                    if (typeof rule.headers !== 'object' || Array.isArray(rule.headers)) {
                        ...
                        Object.entries(rule.headers).forEach(([hk, hv]) => {
                            if (typeof hv !== 'string') {
                                errors.push(`Validation Error: ${base}.headers['${hk}'] must be a string`);
                            }
                        });
                    }
                }
                if (rule.url_type && rule.url_type !== 'regex') {
                    errors.push(`Validation Error: ${base}.url_type must be 'regex' if provided`);
                }
                if (rule.priority !== undefined) {
                    if (typeof rule.priority !== 'number' ||
                        !Number.isInteger(rule.priority) ||
                        !Number.isFinite(rule.priority)) {
                        errors.push(`Validation Error: ${base}.priority must be an integer`);
                    }
                }
            });
        }
