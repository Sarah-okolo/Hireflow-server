...
export async function ApplyTemplate(fileName, apiKey) {
    try {
...
export async function ApplyTemplateLocally(fileName, apiKey) {
    ...
    try {
        const tfContent = getFileContent(fileName).replace('{{API_KEY}}', '"' + apiKey + '"');
        ...
        return new Promise((resolve, reject) => {
            exec('terraform init && terraform apply --auto-approve', { cwd: tempDirPath }, (error, stdout, stderr) => {
                if (error) {
                    reject(`Error: ${stderr || error.message}`);
                }
                else {
                    resolve(`Success: Terraform applied successfully.\nTerraform Output: ${stdout}`);
                }
            });
        });
    }
...
export function extractResourcesAndActions(tfString) {
...
export function getResourceAndAction(fileName) {
    try {
        const fileContent = getFileContent(fileName);
        const resourcesAndActions = extractResourcesAndActions(fileContent);
        if (resourcesAndActions.length > 0 &&
