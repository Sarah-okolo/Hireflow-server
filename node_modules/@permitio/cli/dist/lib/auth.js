...
import { AUTH_API_URL, AUTH_PERMIT_DOMAIN, AUTH_REDIRECT_HOST, AUTH_REDIRECT_PORT, AUTH_REDIRECT_URI, DEFAULT_PERMIT_KEYSTORE_ACCOUNT, KEYSTORE_PERMIT_SERVICE_NAME, AUTH_PERMIT_URL, } from '../config.js';
import { URL, URLSearchParams } from 'url';
import { setTimeout } from 'timers';
const { setPassword, getPassword, deletePassword } = pkg.default;
export var TokenType;
(function (TokenType) {
    TokenType["APIToken"] = "APIToken";
...
export const saveAuthToken = async (token) => {
    try {
        ...
        await setPassword(KEYSTORE_PERMIT_SERVICE_NAME, DEFAULT_PERMIT_KEYSTORE_ACCOUNT, token);
        return '';
    }
    catch (error) {
...
export const loadAuthToken = async () => {
    const token = await getPassword(KEYSTORE_PERMIT_SERVICE_NAME, DEFAULT_PERMIT_KEYSTORE_ACCOUNT);
    if (!token) {
        throw new Error('No token found, use `permit login` command to get an auth token');
    }
    return token;
};
export const cleanAuthToken = async () => {
    await deletePassword(KEYSTORE_PERMIT_SERVICE_NAME, DEFAULT_PERMIT_KEYSTORE_ACCOUNT);
};
export const authCallbackServer = async (verifier) => {
...
export const browserAuth = async () => {
    // Open the authentication URL in the default browser
    function base64UrlEncode(string_) {
        return string_
            .toString('base64')
            .replace(/\+/g, '-')
            .replace(/\//g, '_')
