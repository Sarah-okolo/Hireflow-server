...
export const useApiKeyApi = () => {
    ...
    const getProjectEnvironmentApiKey = useCallback(async (environmentId, projectId, cookie) => {
        return cookie
            ? await unAuthenticatedApiClient(undefined, cookie).GET(`/v2/api-key/{proj_id}/{env_id}`, { proj_id: projectId ?? '', env_id: environmentId })
            : await authenticatedApiClient().GET(`/v2/api-key/{proj_id}/{env_id}`, {
                env_id: environmentId,
            });
    }, [authenticatedApiClient, unAuthenticatedApiClient]);
    const getApiKeyScope = useCallback(async (accessToken) => {
        return accessToken
            ? await unAuthenticatedApiClient(accessToken, undefined).GET(`/v2/api-key/scope`)
            : await authenticatedApiClient().GET(`/v2/api-key/scope`);
    }, [authenticatedApiClient, unAuthenticatedApiClient]);
    const getApiKeyList = useCallback(async (objectType, projectId, accessToken, cookie) => {
        return accessToken || cookie
            ? await unAuthenticatedApiClient(accessToken, cookie).GET(`/v2/api-key`, undefined, undefined, {
                object_type: objectType,
                proj_id: projectId === null ? undefined : projectId,
            })
            : await authenticatedApiClient().GET(`/v2/api-key`, undefined, undefined, {
                object_type: objectType,
                proj_id: projectId === null ? undefined : projectId,
            });
    }, [authenticatedApiClient, unAuthenticatedApiClient]);
    const getApiKeyById = useCallback(async (apiKeyId, accessToken, cookie) => {
        return accessToken || cookie
            ? await unAuthenticatedApiClient(accessToken, cookie).GET(`/v2/api-key/{api_key_id}`, { api_key_id: apiKeyId })
    ...
    const validateApiKeyScope = useCallback(async (apiKey, keyLevel) => {
        let error = null;
        let valid = false;
    ...
    const createApiKey = useCallback(async (body, accessToken, cookie) => {
        return accessToken || cookie
            ? await unAuthenticatedApiClient(accessToken, cookie).POST('/v2/api-key', undefined, body)
