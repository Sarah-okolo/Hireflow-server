...
export const useExport = (apiKey) => {
    ...
    const exportConfig = async (scope) => {
        try {
...
${generateProviderBlock()}`;
            ...
            const userAttributesGenerator = new UserAttributesGenerator(permit, warningCollector);
            const relationGenerator = new RelationGenerator(permit, warningCollector);
            const conditionSetGenerator = new ConditionSetGenerator(permit, warningCollector);
            const resourceSetGenerator = new ResourceSetGenerator(permit, warningCollector);
            const userSetGenerator = new UserSetGenerator(permit, warningCollector);
            const roleDerivationGenerator = new RoleDerivationGenerator(permit, warningCollector);
            ...
            if (userAttributesHCL)
                hcl += userAttributesHCL;
            // Share user attributes with UserSetGenerator if applicable
            if (userAttributesGenerator.userAttributes &&
                userSetGenerator.setUserAttributes) {
                userSetGenerator.setUserAttributes(userAttributesGenerator.userAttributes);
            }
            // Process roles
            ...
            const extendedRoleGenerator = roleGenerator;
            ...
            const extendedRoleDerivationGenerator = roleDerivationGenerator;
            ...
            if (relationsHCL)
                hcl += relationsHCL;
            const extendedRelationGenerator = relationGenerator;
            if (typeof extendedRelationGenerator.getRelationIdMap === 'function') {
                ...
                if (typeof extendedRoleDerivationGenerator.setRelationIdMap === 'function') {
            ...
            if (roleDerivationsHCL)
                hcl += roleDerivationsHCL;
