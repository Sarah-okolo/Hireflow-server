...
export const useGraphDataApi = () => {
    ...
    const fetchGraphData = useCallback(async (projectId, environmentId) => {
        const perPage = 100;
        let currentPage = 1;
        let hasMoreData = true;
        let allResourcesData = [];
        let allRoleAssignmentsData = [];
        const relationsMap = new Map();
        try {
            // Fetch resource instances with pagination
            while (hasMoreData) {
                const resourceResult = await authenticatedApiClient().GET(`/v2/facts/{proj_id}/{env_id}/resource_instances/detailed`, { proj_id: projectId, env_id: environmentId }, undefined, { page: currentPage, per_page: perPage });
                if (resourceResult.error) {
                    throw new Error(resourceResult.error);
                }
                const resourceArray = Array.isArray(resourceResult.data)
                    ? resourceResult.data
                    : resourceResult.data?.data || [];
                const resourcesData = resourceArray.map((res) => ({
            ...
            allResourcesData.forEach(resource => {
                ...
                const mappedRelationships = relationsData.map((relation) => {
            ...
            while (hasMoreData) {
                ...
                const roleResult = await authenticatedApiClient().GET(`/v2/facts/{proj_id}/{env_id}/users`, { proj_id: projectId, env_id: environmentId }, undefined, {
                ...
                const users = Array.isArray(roleResult.data)
                    ? roleResult.data
                    ...
                    if (user.associated_tenants?.length) {
                        user.associated_tenants.forEach(tenant => {
                            if (tenant.resource_instance_roles?.length) {
                                tenant.resource_instance_roles.forEach((resourceInstanceRole) => {
                                    const lookupKey = `${resourceInstanceRole.resource}:${resourceInstanceRole.resource_instance}`;
                                    const resourceInstanceId = id2ToLabelMap.get(lookupKey) ||
