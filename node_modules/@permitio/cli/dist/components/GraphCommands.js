...
export default function Graph() {
    ...
    useEffect(() => {
        if (!scope.project_id || !scope.environment_id) {
            setError('Required project or environment not found in auth context. Please ensure you are logged in with the proper scope.');
        }
    ...
    useEffect(() => {
        const fetchData = async () => {
            ...
            try {
                setLoading(true);
                const { data: graphData, error: fetchError } = await fetchGraphData(scope.project_id, scope.environment_id);
                if (fetchError) {
    ...
    if (authError || error) {
        return React.createElement(Text, { color: "red" }, authError || error);
    }
    if (noData) {
        return React.createElement(Text, null, "Environment does not contain any data");
    }
    return React.createElement(Text, null, "Graph generated successfully and saved as HTML!");
}
