...
export default function CreateEnvComponent({ name: initialName, envKey: initialKey, description: initialDescription, customBranchName: initialCustomBranchName, jwks: initialJwks, settings: initialSettings, }) {
    const { scope } = useAuth();
    ...
    const fields = useMemo(() => [
        ...
        },
        {
            ...
            required: true,
            helpText: 'Keys should only contain lowercase letters, numbers, and underscores.',
        ...
        try {
            ...
            if (formState.jwks) {
                try {
                    ...
                    setError(`Invalid JWKS JSON: ${e instanceof Error ? e.message : String(e)}`);
                    setStatus('error');
                    return;
                }
            }
            if (formState.settings) {
                try {
                    ...
                    setError(`Invalid settings JSON: ${e instanceof Error ? e.message : String(e)}`);
                    setStatus('error');
                    return;
                }
            }
            // Call API to create environment
            const result = await createEnvironment(scope.project_id, undefined, null, requestBody);
            if (result.error) {
    ...
    useEffect(() => {
        ...
        const firstEmptyFieldIndex = fieldOrder.findIndex(fieldId => !initialValues[fieldId]);
    ...
    const renderCurrentField = () => {
        ...
        if (formState.currentField < 0 || formState.currentField >= fields.length) {
            return (React.createElement(Box, null,
                React.createElement(Text, { color: "red" }, "Error: Invalid form state. Please try again.")));
        }
        // Use non-null assertion since we've verified the index is valid
        const field = fields[formState.currentField];
        const currentValue = formState[field.id] || '';
        return (React.createElement(Box, { flexDirection: "column" },
            React.createElement(Text, null, field.label),
            React.createElement(TextInput, { value: currentValue, onChange: handleInputChange, onSubmit: handleFieldSubmit }),
            field.helpText ? React.createElement(Text, { dimColor: true }, field.helpText) : null,
    ...
    if (status === 'success') {
        return (React.createElement(Box, { flexDirection: "column" },
            React.createElement(Text, null, "\u2705 Environment created successfully!"),
            React.createElement(Text, null,
                "Environment ID: ",
            ...
            React.createElement(Text, null,
                "Key: ",
                formState.key),
            formState.description && (React.createElement(Text, null,
                "Description: ",
                formState.description)),
            formState.customBranchName && (React.createElement(Text, null,
                "Custom Branch Name: ",
                formState.customBranchName)),
            formState.jwks && React.createElement(Text, null, "JWKS: [JSON Web Key Set provided]"),
            formState.settings && (React.createElement(Text, null, "Settings: [Custom settings provided]"))));
    }
    // Error state
    return (React.createElement(Box, { flexDirection: "column" },
        React.createElement(Text, { color: "red" },
            "Error: ",
            error),
        React.createElement(Text, null, "Try again or use --help for more information.")));
}
