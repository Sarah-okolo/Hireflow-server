...
import { globalScopeGetterSetter, globalTokenGetterSetter, } from '../hooks/useClient.js';
...
export function AuthProvider({ children, permit_key: key, scope, skipLogin, }) {
    const { validateApiKeyScope, getApiKeyList, getApiKeyById, createApiKey } = useApiKeyApi();
    const { authSwitchOrgs } = useAuthApi();
    const [internalAuthToken, setInternalAuthToken] = useState(null);
    ...
    useEffect(() => {
        ...
        const fetchAuthToken = async (redirect_scope) => {
            try {
                const token = await loadAuthToken();
                const { valid, scope: keyScope, error, } = await validateApiKeyScope(token, redirect_scope === 'login' ? 'environment' : redirect_scope);
                if (error || !valid) {
    ...
    useEffect(() => {
        if (state === 'validate') {
            (async () => {
                const { valid, scope: keyScope, error, } = await validateApiKeyScope(key ?? '', scope ?? 'environment');
                if (!valid || error) {
    ...
    const switchActiveOrganization = useCallback(async (organization_id) => {
        const { headers, error } = await authSwitchOrgs(organization_id, internalAuthToken, cookie);
        if (error) {
    ...
    useEffect(() => {
        if ((state === 'organization' && organization) ||
            ...
            (async () => {
                const { data: response, error } = await getApiKeyList(state === 'organization' ? 'org' : 'project', project, internalAuthToken ?? '', newCookie);
                if (error) {
                    setError(`Error while getting api key list ${error}`);
                    return;
                }
                let cliApiKey = response?.data.find(apiKey => apiKey.name === 'CLI_API_Key');
                if (!cliApiKey && organization) {
                    ...
                    const { data: creationResponse, error: creationError } = await createApiKey(body, internalAuthToken, newCookie);
                    if (creationError) {
                ...
                const { data: secret, error: err } = await getApiKeyById(apiKeyId, internalAuthToken ?? '', newCookie);
                if (err) {
    ...
    const handleLoginSuccess = useCallback((_organisation, _project, _environment, secret) => {
        setAuthToken(secret);
        setCurrentScope({
    ...
    return (React.createElement(React.Fragment, null,
        state === 'loading' && React.createElement(Text, null, "Loading Token"),
        (state === 'organization' || state === 'project') && (React.createElement(React.Fragment, null,
            React.createElement(LoginFlow, { onSuccess: onLoginSuccess, onError: setError }),
            internalAuthToken && cookie && !organization && (React.createElement(SelectOrganization, { accessToken: internalAuthToken, onComplete: async (organization) => {
                    setOrganization(organization.value);
                    await switchActiveOrganization(organization.value);
                }, onError: setError, cookie: cookie })),
            state === 'project' &&
                internalAuthToken &&
                newCookie &&
                organization &&
                !project && (React.createElement(SelectProject, { accessToken: internalAuthToken, onComplete: project => setProject(project.value), cookie: newCookie, onError: setError })))),
        state === 'login' && (React.createElement(React.Fragment, null,
            React.createElement(Login, { options: {}, loginSuccess: handleLoginSuccess }))),
        state === 'creating-key' && (React.createElement(React.Fragment, null,
            React.createElement(Text, null, "CLI_API_Key not found, creating one for you."))),
        state === 'done' &&
            ((authToken && currentScope) || (skipLogin && currentScope)) && (React.createElement(React.Fragment, null,
            keyCreated && (React.createElement(React.Fragment, null,
                React.createElement(Text, null,
                    ...
                    currentScope.environment_id
                        ...
                        : currentScope.project_id
                            ...
                            : 'organization',
                    ' ',
                    "level key for you, named CLI_API_key (this key is protected, please do not change it)"),
